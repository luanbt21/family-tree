...
async function checkTreeAccess(db: PrismaClient, userId: string, treeId: string, allowedRoles: string[] = ['OWNER', 'EDITOR', 'VIEWER']) {
...
export const createEdge = command(z.any(), async (payload: any) => {
	...
	if (sourceNode.treeId !== targetNode.treeId) throw new Error('Nodes must belong to the same family tree.');
