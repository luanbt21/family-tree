...
<Popover.Root bind:open>
  ...
  <Popover.Content
    class="w-36 p-1 border rounded-lg gap-0 bg-background/95 backdrop-blur-md"
    align="end"
  >
    <div class="py-0.5 space-y-0.5" role="menu" aria-orientation="vertical">
      {#each locales as locale}
        <button
          type="button"
          class="flex w-full items-center justify-between px-3 py-2 text-xs rounded-md transition-colors {currentLocale ===
          locale
